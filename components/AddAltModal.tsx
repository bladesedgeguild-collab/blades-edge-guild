...
export function AddAltModal({
  ...
  const excludeIdsJson = JSON.stringify(excludeIds ?? [])

  useEffect(() => {
    if (query.length < 1) { setResults([]); setSearching(false); return }
    setSearching(true)
    const excludeSet = new Set<string>(JSON.parse(excludeIdsJson))
    const t = setTimeout(async () => {
      ...
      setResults(unique.filter((c) => !c.claimed_by && !excludeSet.has(c.id)))
      setSearching(false)
    }, 300)
    return () => clearTimeout(t)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [query, excludeIdsJson])
  ...
  const getColumns = (items: SearchChar[]) => {
    ...
    for (let i = 0; i < numCols; i++) cols.push(items.slice(i * perCol, (i + 1) * perCol))
    return cols
  }
  ...
  const renderResultRow = (char: SearchChar) => {
    ...
    return (
      ...
      >
        <div className="alt-result-avatar" style={selected ? { background: 'rgba(26,18,8,0.2)', border: '1px solid rgba(26,18,8,0.4)', color: '#1a1208' } : { background: `${color}22`, border: `1px solid ${color}99`, color }}>
          {char.name.charAt(0)}
        </div>
      ...
      <div className="alt-search-modal">
        ...
        {step === 'search' && (
          <div>
            ...
            {selectedChars.length > 0 && (
              <div className="alt-selected-tray">
                {selectedChars.map(char => (
                  <div key={char.id} className="alt-selected-chip" style={{ borderColor: CLASS_COLORS[char.class] ?? '#c9961a' }}>
                    ...
                    <button className="alt-selected-remove" onClick={() => toggleSelect(char)} aria-label={`Remove ${char.name}`}>×</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ position: 'relative', marginBottom: 8 }}>
              ...
              {results.length > 0 && (
                <div onMouseDown={(e) => e.preventDefault()} style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 99999, backgroundColor: 'var(--be-bg-1)', border: '1px solid rgba(201,150,26,0.3)', borderRadius: 8, boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
                  <div className="alt-results-grid" style={{ gridTemplateColumns: `repeat(${columns.length}, 1fr)` }}>
            ...
            <button className="alt-confirm-btn" onClick={handleAddAlts} disabled={selectedChars.length === 0 || loading}>
              {loading ? 'Adding…' : selectedChars.length === 0 ? 'Select characters above' : `Add ${selectedChars.length} Alt${selectedChars.length > 1 ? 's' : ''} →`}
            </button>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input type="text" autoComplete="off" value={form.name} maxLength={12}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: 4 }}>
                  ...
                  <span style={{ fontFamily: 'var(--be-font-body)', fontSize: '0.72rem', color: form.name.length >= 12 ? 'var(--be-gold)' : 'rgba(138,122,90,0.5)', flexShrink: 0, marginLeft: 8 }}>{form.name.length} / 12</span>
                </div>
              </div>
              <div>
              ...
              <button
                onClick={() => { const e = validateForm(form); setErrors(e); if (Object.keys(e).length === 0) setStep('newConfirm') }}
